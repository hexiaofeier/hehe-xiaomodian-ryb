...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/style-black%20%26%20white%20with%20flexible%20color-f97316.svg" alt="Black and white with flexible character color">
</p>

> 把照片、角色设想或游戏截图，转成**自然比例、纯白底、黑白细线**的单幅角色插画；浅色肤色的脸部保持白色留白，服装、固有色和阴影则按角色灵活处理。
...
| 画面 | 角色 | 留白 | 点色 |
| --- | --- | --- | --- |
| 清晰的黑色手绘线 | 自然头身与可信结构 | 纯白、无纹理背景 | 红黄蓝优先，固有色可灵活保留 |

- **人和动物都要站得住。** 不做大头小身或幼态 Q 版。
- **脸部留白是硬标准。** 浅色肤色的脸部不铺肉色、粉色或米色；黑色、深棕色或特殊肤色按角色设定保留干净底色。
- **衣物按角色处理。** 可以纯黑白、全灰、局部点色，也可保留较大面积的服装固有色。
- **阴影没有固定配额。** 可以极简，也可以较丰富；只要结构清楚、主体不脏不糊，且服务于角色。
- **非人角色保留弹性。** 例如哆啦A梦可以保留全蓝身体，也可以转为全灰。
...
人物保持与年龄、体型相符的自然比例；若基于照片或截图，保留【脸型、五官位置、发型、服装、道具或动物结构】，
但重画成简洁的日式漫画角色。纯白无纹理背景，以清楚的黑色手绘线和黑白灰关系为视觉基底。
浅色肤色的脸部不得填肉色、粉色或米色，必须以白色留白为主；黑色、深棕色或特殊肤色按角色设定保留干净底色。
服装、道具和非人身体可以纯黑白、全灰、局部点色，也可保留较大面积的固有色；红黄蓝优先但不强制。
阴影的数量、面积和深浅按角色、服装结构与画面感觉决定。不要无意义的 Q 版、浅色脸部铺肤色、杂乱背景、文字或水印。
~~~

## 公开版边界

本仓库不包含用户的原始照片或原始游戏截图。<code>assets/examples/</code> 与 <code>skills/hehe-xiaomodian-ryb/assets/</code> 中的图均为 Codex 生成的示例或运行时风格参考，用于说明提示词与画面控制方向。示例中涉及的角色、作品名称和相关权利归各自权利人所有；使用者应确保自己提供的参考材料具有合法使用权限。
